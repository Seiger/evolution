<?php
/*List pack */
$_Lang['version'] = "Версія";
$_Lang['author'] = "Автор";
$_Lang['date'] = "Випущено";
$_Lang['downloads'] = "Завантажено";
$_Lang['install'] = "Встановити";
$_Lang['reinstall'] = "Перевстановити";
$_Lang['update'] = "Оновити";
$_Lang['installed'] = "Встановлено";
$_Lang['installed2'] = "Встановлено";
$_Lang['exit'] = "Вихід";
$_Lang['alert_overwrite'] = "Встановлення цього доповнення перезапише існуючі. Продовжити?";

/*Main*/
$_Lang['store_name'] = "Управління пакетами";
$_Lang['version_evailble'] = "Доступна нова версія";
$_Lang['category'] = "Категорії";

/*Login form*/
$_Lang['enter_in_own_repository'] = "Увійти до свого репозиторію";
$_Lang['password'] = "Пароль";
$_Lang['enter'] = "Увійти";
$_Lang['email'] = "Email";
$_Lang['register'] = "Зареєструватися";
$_Lang['own_repository'] = "Мій репозиторій";
$_Lang['login_errors'] = "Неправильний Email або пароль";

/* &tpl filters */
$_Lang['s1_1_reated_on'] = "Дата створення";
$_Lang['s1_2_name'] = "Назвою";
$_Lang['s1_3_position'] = "Позицією";
$_Lang['s1_4_published'] = "Опубліковано";
$_Lang['s2_1_up'] = "За спаданням";
$_Lang['s2_2_down'] = "За зростанням";
$_Lang['search'] = "Пошук по категорії: ";

/* Installation by file-upload */
$_Lang['install_file'] = "Встановлення з архіву";
$_Lang['choose_file_msg'] = "Виберіть файл";
$_Lang['install_file_btn'] = "Встановити";
$_Lang['install_file_success'] = "Встановлення завершено";

/* faq */
$_Lang['faq'] = '
<li><a href="https://extras.evo.im/faq/whatfor.html" target="_blank">Навіщо свій репозиторій</a></li>
<li><a href="https://extras.evo.im/faq/howtoadd.html" target="_blank">Як додати пакет у репозиторій</a></li>
<li><a href="https://extras.evo.im/faq/howtocollect.html" target="_blank">Як правильно зібрати пакет для встановлення</a></li>';
?>
